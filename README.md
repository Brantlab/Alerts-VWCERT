...
- Selecting an active **Tornado Warning** opens a red operations panel. It records which of the county's nine listed sirens were activated, runs a three-minute siren countdown with an automatic completion popup, and shows the live elapsed time since the last logged radio broadcast. These details are included in the activity report and JSON export.
